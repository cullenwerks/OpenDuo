...
impl CicdTools {
    pub fn all(client: GitLabClient) -> Vec<Box<dyn Tool>> {
        vec![
            Box::new(GetPipelineYaml {
                client: client.clone(),
            }),
            Box::new(ValidatePipelineYaml {
                client: client.clone(),
            }),
            Box::new(ListRunners {
                client: client.clone(),
...
impl Tool for GetPipelineYaml {
    ...
    fn description(&self) -> &str {
        "Get the .gitlab-ci.yml pipeline configuration for a project."
    }
...
impl Tool for ValidatePipelineYaml {
    ...
    fn description(&self) -> &str {
        "Validate a .gitlab-ci.yml configuration."
    }
    fn parameters_schema(&self) -> Value {
        json!({
            "type": "object",
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        let v: Value = self
            .client
            .post("ci/lint", json!({ "content": args["content"] }))
            .await?;
