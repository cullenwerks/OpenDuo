...
impl Tool for GetFile {
    ...
    fn description(&self) -> &str {
        "Get a file's content from a GitLab repository."
    }
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        let file_path = urlencoding::encode(
            args["file_path"]
                .as_str()
                .ok_or_else(|| anyhow::anyhow!("file_path required"))?,
        );
        let git_ref = urlencoding::encode(args["ref"].as_str().unwrap_or("main"));
        let v: Value = self
            .client
            .get(&format!(
                "projects/{}/repository/files/{}?ref={}",
                pid, file_path, git_ref
            ))
            .await?;
...
impl Tool for ListFiles {
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        let path = urlencoding::encode(args["path"].as_str().unwrap_or(""));
        let git_ref = urlencoding::encode(args["ref"].as_str().unwrap_or("main"));
...
impl Tool for GetCommit {
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        let sha = urlencoding::encode(
            args["sha"]
                .as_str()
                .ok_or_else(|| anyhow::anyhow!("sha required"))?,
        );
        let v: Value = self
            .client
            .get(&format!("projects/{}/repository/commits/{}", pid, sha))
            .await?;
        Ok(serde_json::to_string_pretty(&v)?)
    }
}
...
impl Tool for CompareRefs {
    ...
    async fn execute(&self, args: Value) -> Result<String> {
        ...
        let from = urlencoding::encode(
            args["from"]
                .as_str()
                .ok_or_else(|| anyhow::anyhow!("from required"))?,
        );
        let to = urlencoding::encode(
            args["to"]
                .as_str()
